...
from px4_msgs.msg import OffboardControlMode, TrajectorySetpoint, VehicleCommand, VehicleLocalPosition, VehicleControlMode, VehicleGlobalPosition, VehicleStatus


class OffboardNorth1m(Node):
    """在offboard模式下控制无人机向正北方平移1米的节点。"""

    def __init__(self) -> None:
        super().__init__('offboard_north_1m')
        ...
        self.declare_parameter('north_distance', 1.0)  # 向正北平移的距离（米）
        ...
        self.north_distance = self.get_parameter('north_distance').value
        ...
        if not self.headless:
            self.get_logger().info(f"参数配置: headless={self.headless}, target_altitude={self.target_altitude}, north_distance={self.north_distance}")
        ...
        if is_offboard:
            ...
            if self.offboard_entry_time is None:
                ...
                if elapsed_time >= self.offboard_maintain_time and not self.offboard_mode_maintained:
                    ...
                    if not self.headless:
                        self.get_logger().info(f"offboard模式已维持 {self.offboard_maintain_time} 秒，准备执行向北平移命令")
        else:
            # 退出offboard模式，重置计时
            ...
            if not self.task_completed and hasattr(self.vehicle_local_position, 'x'):
                ...
                if distance_to_target < self.position_tolerance:
                    ...
                    if self.task_start_time and not self.headless:
                        ...
                        self.get_logger().info(f"✅ 任务完成！已成功向正北平移1米到达目标位置")
...
def main(args=None) -> None:
    print('启动offboard向正北1米控制节点...')
    rclpy.init(args=args)
    offboard_north_1m = OffboardNorth1m()
    rclpy.spin(offboard_north_1m)
    offboard_north_1m.destroy_node()
    rclpy.shutdown()
