...
from px4_msgs.msg import VehicleLocalPosition, VehicleCommandAck, VehicleAttitude, VehicleControlMode
...
class PX4ConnectionDiagnostics(Node):
    ...
    def timer_callback(self) -> None:
        ...
        if not self.position_received and not self.attitude_received and not self.control_mode_received:
        ...
        elif not self.position_received:
            self.get_logger().warn("⚠ VehicleLocalPosition话题未连接")
            self.get_logger().warn("  可能原因: PX4飞控位置估计系统未初始化或通信问题")
